...
import { findMatchGroups } from "./matchDetection";
...
describe("generateBoard", () => {
  it("produces a board with the correct dimensions", () => {
    const board = generateBoard(6, 6, [R, O, Y]);
  ...
  it("fills all active cells with valid candy types", () => {
    const types = [R, O, Y];
    const board = generateBoard(7, 7, types);
    board.cells
      .filter((c) => c.isActive)
      .forEach((c) => {
  ...
  it("does not produce initial 3-in-a-row matches", () => {
    const board = generateBoard(8, 8, [R, O, Y, G]);
  ...
  it("marks inactive cells correctly based on gridShape", () => {
    ...
    const board = generateBoard(3, 3, [R, O, Y], shape);
  ...
  it("leaves inactive cells empty", () => {
    ...
    const board = generateBoard(2, 2, [R, O], shape);
    board.cells.forEach((c) => {
      if (!c.isActive) expect(c.candyType).toBe("");
    });
  });
});
...
describe("applyGravity", () => {
  ...
  it("does not touch inactive cells", () => {
    const cells: Cell[] = [
      makeCell(R),
      { candyType: "", isActive: false, specialType: SpecialType.None },
      makeCell(B),
    ];
    const board: Board = { width: 1, height: 3, cells };
    const result = applyGravity(board);
...
describe("fillEmptyCells", () => {
  it("fills empty active cells with a candy from the provided pool", () => {
    ...
    const result = fillEmptyCells(board, [R, O, Y]);
  ...
  it("does not overwrite non-empty cells", () => {
    ...
    const result = fillEmptyCells(board, [O]);
  ...
  it("does not fill inactive cells", () => {
    ...
    const board: Board = { width: 1, height: 1, cells };
    const result = fillEmptyCells(board, [R]);
...
describe("hasValidMoves", () => {
  it("detects a valid horizontal move", () => {
    ...
    expect(hasValidMoves(board2)).toBe(true);
  });
...
describe("shuffleBoard", () => {
  it("produces a board with at least one valid move", () => {
    ...
    const shuffled = shuffleBoard(board, [R, O, Y]);
    expect(hasValidMoves(shuffled)).toBe(true);
  });

  it("preserves the same multiset of candy types", () => {
    const board = generateBoard(6, 6, [R, O, Y]);
    const original = board.cells
      .filter((c) => c.isActive)
      .map((c) => c.candyType)
      .sort();
    const shuffled = shuffleBoard(board, [R, O, Y]);
    const result = shuffled.cells
      .filter((c) => c.isActive)
      .map((c) => c.candyType)
      .sort();
...
describe("applyCascade", () => {
  it("clears a simple 3-match and returns score", () => {
    ...
    const { board: result, totalScore } = applyCascade(
      board,
      [O, Y, G],
      30,
      1.5,
  ...
  it("applies cascade multiplier on subsequent cascades", () => {
    ...
    const { totalScore } = applyCascade(board, [Y, G, B], 30, 1.5);
    expect(totalScore).toBeGreaterThan(0);
  });
  ...
  it("returns the board unchanged when there are no matches", () => {
    ...
    expect(result.cells.map((c) => c.candyType)).toEqual(
      board.cells.map((c) => c.candyType),
    );
  });
});
