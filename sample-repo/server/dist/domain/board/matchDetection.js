...
exports.findMatchGroups = findMatchGroups;
exports.getMatchedIndices = getMatchedIndices;
exports.findWrappedCandyIndices = findWrappedCandyIndices;
...
function findMatchGroups(board) {
    ...
    for (let row = 0; row < height; row++) {
        ...
        for (let col = 1; col <= width; col++) {
            ...
            const runContinues = currCell !== null &&
                currCell.isActive &&
                prevCell.isActive &&
                prevCell.candyType !== "" &&
            ...
            if (!runContinues) {
                ...
                if (runLength >= 3 && startCell.isActive && startCell.candyType !== "") {
                    const indices = [];
 ...
 */
function getMatchedIndices(groups) {
 ...
 */
function determineSpecialForRun(runLength) {
    if (runLength >= 5)
        return shared_1.SpecialType.ColorBomb;
    if (runLength === 4)
        return shared_1.SpecialType.Striped;
    return shared_1.SpecialType.None;
}
/**
 ...
 */
function findWrappedCandyIndices(groups, boardWidth) {
    const horizontalIndices = new Set();
    const verticalIndices = new Set();
    for (const group of groups) {
